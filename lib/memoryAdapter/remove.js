...
function deleteFiles(db, files) {
	files.forEach(function (fileName) {
		var index = db.files.paths[fileName];
...
function deleteGroups(db, groups) {
	groups.forEach(function (groupName) {
		var index = db.groups.names[groupName];
		db.groups.items.splice(index, 1);
		delete db.groups.names[groupName];
		db.files.items = db.files.items.map(function (file) {
			var groupIndex = file.parents.indexOf(groupName);
			if (groupIndex >= 0) {
...
function removeGroups(db, groups, remove) {
	...
	return groups.filter(function (groupName) {
		return remove.indexOf(groupName) < 0;
	});
}
...
function removeFiles(db, files, remove) {
	...
	return files.filter(function (fileName) {
		return remove.indexOf(fileName) < 0;
	});
}

function remove(db, groupName, files, groups, cb) {
	...
	var group = db.groups.items[db.groups.names[groupName]];
